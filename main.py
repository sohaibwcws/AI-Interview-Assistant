...
GOOGLE_APPLICATION_CREDENTIALS = "Google Speech to text.json"
...
class TranscriptionApp(ctk.CTk):
    BACKGROUND_INFO = "Your Experience"
    ...
    def setup_text_widgets(self):
        ...
        # Display background info in the transcription text
        self.transcription_text.insert("end", "Background Information:\n" + self.BACKGROUND_INFO + "\n\n")
    ...
    def setup_buttons(self):
        ...
        self.type_transcription_label = tk.Label(self, text="Type or edit transcription:")
        self.type_transcription_label.pack(pady=5)

        self.transcription_entry = tk.Entry(self, width=50)
        self.transcription_entry.pack(pady=5)

        self.submit_transcription_button = ctk.CTkButton(self, text="Submit Transcription", command=self.submit_transcription)
        self.submit_transcription_button.pack(pady=10)

    def submit_transcription(self):
        typed_transcription = self.transcription_entry.get()
        if typed_transcription:
            self.handle_transcription(typed_transcription)
            self.transcription_entry.delete(0, "end")
    ...
    def generate_response(self, reasoning):
        ...
        try:
            # Add a system message with the reasoning
            messages.append({"role": "system", "content": reasoning})
            ...
            self.save_to_database(self.chat_history[-2]['content'], response_text)
